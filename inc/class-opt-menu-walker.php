...
/** Custom menu walker **/
class _PF_Opt_Menu_Walker extends Walker_Nav_Menu {
...
//var_dump($element);
	...
	function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		if(is_array($pmm_background)){
			$item_bg = 'background: white '.( '' !== $pmm_background['image'][0] ? 'url('._PF_Images::thumbnail(array("source"=>$pmm_background['image'][0], "echo"=>0, "src"=>1)).') ' : '').( '' !== $pmm_background['repeat'][0] ? $pmm_background['repeat'][0].' ' : '').( '' !== $pmm_background['position'][0] ? $pmm_background['position'][0] : '').';';
		}else{
			$item_bg = '';
		}
    ...
    public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if('true' === $pmm_header && 'true' === $args->mega_child){
			...
			$item_output .= '<a'. $attributes .'>';
			$item_output .= _PF_Images::thumbnail($imgargs);//'<img src="'.get_post_meta($item->ID, 'pmm_image', 1).'"/>';//
...
add_filter( 'wp_nav_menu_args' , 'electron_add_menu_walker' );
function electron_add_menu_walker( $args ) {
	if ( $args['container_class'] == 'proton-mega-menu' ) {
		$args['walker'] = new _PF_Opt_Menu_Walker;
	}
